...
const BookCard: React.FC<Props> = ({ cover, title, author,Okey }) => {
  ...
  return (
    <div onClick={()=>{
    ...
    }} className="group relative flex flex-col bg-white rounded-lg shadow-md cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 h-[35dvh] w-[15dvw] min-w-[180px] max-w-[220px] overflow-hidden">
      {/* Book Cover */}
      <div className="h-[75%] w-full flex items-center justify-center bg-gray-100 relative overflow-hidden">
        {loading && <Spinner />}
      ...
      {/* Book Info */}
      <div className="h-[25%] w-full p-2 flex flex-col justify-center bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
        <h1 
